...
(function () {
  ...
  /* ------------------------------------------------------------------
     ADMIN MASTER CODE
     Scanning this stamps every station at once, and scanning it again on an
     already-complete passport clears the device. It is for the organiser's own
     testing — there is no reset button in the app, so this doubles as the undo.

     WARNING: a QR code carries plain text and nothing else, so this string is
     the entire secret. `all_qrcode` is guessable — a student who wonders "what
     if I make a QR that says all_qrcode" can generate one in a browser in ten
     seconds and claim a prize without visiting a single station. If that
     matters, change this to something unguessable, e.g.
         'all_qrcode_h7q2xk49'
     and re-run `python make_qr_codes.py --force` to reprint it.
     ------------------------------------------------------------------ */
  var ADMIN_CODE = 'all_qrcode';
  ...
  function stampEverything() {
    ...
    BASES.forEach(function (b) { if (!data.stamps[b.id]) data.stamps[b.id] = now; });
    writeStore(data);
  }
  function clearEverything() { writeStore({ v: 2, stamps: {} }); }
  ...
  function handleCode(raw) {
    ...
    /* Admin master code, checked before the station table so it can never be
       shadowed by a station id. */
    if (text.toLowerCase() === ADMIN_CODE.toLowerCase()) {
      lastHandled = text;
      /* This code toggles, so a lingering camera would silently undo what it
         just did. Hold it off well past the time it takes to lower the phone. */
      cooldownUntil = performance.now() + 5000;
      var wasComplete = stampedCount() === TOTAL;

      if (wasComplete) {
        clearEverything();
        renderAll();
        setMode('warn', 'ADMIN RESET');
        setResult('dup', '⟲', 'ล้างแสตมป์ทั้งหมดแล้ว (admin)',
          'เครื่องนี้กลับไปเป็น 0/' + TOTAL + ' — สแกนรหัสนี้อีกครั้งเพื่อเติมให้ครบ');
        notify('ล้างแสตมป์ทั้งหมดแล้ว', 'admin · เครื่องนี้กลับไปเป็น 0/' + TOTAL, 'warn');
        beep(300, 150);
        buzz([50, 40, 50]);
      } else {
        stampEverything();
        ...
        setMode('ok', 'ADMIN');
        setResult('ok', '★', 'เติมแสตมป์ครบทุกฐานแล้ว (admin)',
          TOTAL + '/' + TOTAL + ' ผ่านเกณฑ์ทั้งสองกลุ่ม — สแกนรหัสนี้อีกครั้งเพื่อล้างทั้งหมด');
        notify('เติมครบ ' + TOTAL + '/' + TOTAL + ' แล้ว', 'admin · สแกนซ้ำอีกครั้งเพื่อล้างทั้งหมด', 'ok');
